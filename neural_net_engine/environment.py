# ======================================================
# As of now, code heavily copied from 
...
class ChessEnv:
    ...
    ** Board encoding information **
        ...
        The 8 * 14 planes encode the 8 most recent board positions, here referred to as the
            board's 'history', grouped into sets of 14 planes per position.

            The first 6 planes in a set of 14 denote the pieces of the active player (the player that moves). 
                Each of the 6 planes is associated w/ a particular piece type (pawn, knight, bishop, queen, king). 
            The next 6 places encode the pieces of the opponent player, following the same scheme. 
    ...
    ** Move encoding information **
        ...
        Moves are encoded as indices into a flattened 8 x 8 x (8 * 7 + 8 + 9) array, 
            where each position encodes a possible move.
        The first 2 dimensions correspond to the square from which the piece is picked up. 
    ...
    def reset(self):
        ...
        return self._encode_board(self._board.copy())


    # =======================================================
    # code for implementing a chess game
    # =======================================================
    def play(self, move):
        # either string input or chess.Move input
        if type(move) == str:
            move = chess.Move.from_uci(move)

        if type(move) == chess.Move:
            move = self._encode_move(move)
        else:
    ...
    def result(self):
        res = self._board.outcome()

        if res is not None:
            if res.result() == '1/2-1/2':
                print(f'Game ended by {res.termination}')
                print(f'Result: {res.result()}; Draw!!')
            else:
                winner = 'White' if res.winner == chess.WHITE else 'Black'
                print(f'Game ended by {res.termination}')
                print(f'Result: {res.result()}; {winner} wins!!')
        else:
            print('Error in ending game; cannot compute result...')

    def logResult(self, games_count):
        outcome = self._board.outcome()
        if outcome is None:
            logging.error(f'{games_count}:None,None,None')
        else:
            if outcome.result() == '1/2-1/2':
                logging.info(f'{games_count}:None,{outcome.result()},{outcome.termination}')
            else:
                winner_color = 'white' if outcome.winner == chess.WHITE else 'black'
                logging.info(f'{games_count}:{winner_color},{outcome.result()},{outcome.termination}')

    def logTraining(self, games_count, timestep_score, epsilon_val):
        outcome = self._board.outcome()
        if outcome is None:
            logging.error(f'{games_count}:None,None,None,None,None')
        else:
            if outcome.result() == '1/2-1/2':
                logging.info(f'{games_count}:None,{outcome.result()},{timestep_score},{outcome.termination},{epsilon_val}')
            else:
                winner_color = 'white' if outcome.winner == chess.WHITE else 'black'
                logging.info(f'{games_count}:{winner_color},{outcome.result()},{timestep_score},{outcome.termination},{epsilon_val}')


    # =======================================================
    # code for implementing environment to train DQL model
    # =======================================================
    def step(self, action):
        ...
        next_state = self._encode_board(self._board.copy())
        reward = self._reward()
        done = self._board.is_game_over()
        self._ready = not done
    ...
    def _legal_actions(self):
        return np.array([self._encode_move(move) for move in self._board.legal_moves])
    ...
    def _decode_move(self, action):
        # converts an integer action to the corresponding `chess.Move` object for the current board position
